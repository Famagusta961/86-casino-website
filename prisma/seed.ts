...
const photos = {
  hero: "https://images.unsplash.com/photo-1518893883800-45cd0954574b?auto=format&fit=crop&w=1800&q=85",
  roulette: "https://images.unsplash.com/photo-1596838132731-3301c3fd4317?auto=format&fit=crop&w=1000&q=80",
  cards: "https://images.unsplash.com/photo-1605870445919-838d190e8e1b?auto=format&fit=crop&w=1000&q=80",
  chips: "https://images.unsplash.com/photo-1596838132731-3301c3fd4317?auto=format&fit=crop&w=1000&q=80",
  lounge: "https://images.unsplash.com/photo-1548811579-017e9f3a4bbf?auto=format&fit=crop&w=1200&q=80",
  dj: "https://images.unsplash.com/photo-1571266028243-d220c9c3b0a9?auto=format&fit=crop&w=1000&q=80",
  champagne: "https://images.unsplash.com/photo-1510812431401-41d2bd2722f3?auto=format&fit=crop&w=1000&q=80",
...
async function main() {
  ...
  for (let i = 0; i < games.length; i++) {
    ...
    await prisma.game.upsert({ where: { id: `seed-game-${i}` }, update: {}, create: { id: `seed-game-${i}`, name, subtitle, description, image, sortOrder: i } });
  }
  const promotions = [
  ...
  for (let i = 0; i < promotions.length; i++) { const [title, subtitle, description, image] = promotions[i]; await prisma.promotion.upsert({ where: { id: `seed-promo-${i}` }, update: {}, create: { id: `seed-promo-${i}`, title, subtitle, description, image, featured: true, sortOrder: i } }); }
  const events = [
  ...
  for (let i = 0; i < events.length; i++) { const [title, subtitle, description, image] = events[i]; await prisma.event.upsert({ where: { id: `seed-event-${i}` }, update: {}, create: { id: `seed-event-${i}`, title, subtitle, description, image, dateTime: new Date(Date.now() + (i + 1) * 86400000), sortOrder: i } }); }
  const pages = [
  ...
  const gallery = [photos.hero, photos.roulette, photos.lounge, photos.dj, photos.champagne, photos.exterior];
  for (let i = 0; i < gallery.length; i++) await prisma.galleryItem.upsert({ where: { id: `seed-gallery-${i}` }, update: {}, create: { id: `seed-gallery-${i}`, image: gallery[i], altText: "86 CASINO experience", category: i % 2 ? "gaming" : "atmosphere", sortOrder: i } });
}
